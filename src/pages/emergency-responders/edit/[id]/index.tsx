...
import { getEmergencyResponderById, updateEmergencyResponderById } from 'apiSdk/emergency-responders';
...
function EmergencyResponderEditPage() {
  ...
  const handleSubmit = async (values: EmergencyResponderInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateEmergencyResponderById(id, values);
      mutate(updated);
      resetForm();
