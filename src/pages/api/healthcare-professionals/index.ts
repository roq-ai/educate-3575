...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  switch (req.method) {
    case 'GET':
      return getHealthcareProfessionals();
    case 'POST':
      return createHealthcareProfessional();
    default:
      return res.status(405).json({ message: `Method ${req.method} not allowed` });
  }

  async function getHealthcareProfessionals() {
    const data = await prisma.healthcare_professional
      .withAuthorization({
        roqUserId,
        tenantId: user.tenantId,
        roles: user.roles,
      })
      .findMany(convertQueryToPrismaUtil(req.query, 'healthcare_professional'));
  ...
  async function createHealthcareProfessional() {
    ...
    const data = await prisma.healthcare_professional.create({
      data: body,
    });
    return res.status(200).json(data);
  }
